from data_utils.sql_datasets import SQLDataset
...
def prepare_dataset_main(args, tokenizer):
    data = {}
    data["test"] = SQLDataset(args, tokenizer, "test", args.data_dir, args.dev_num)

    return data


def run_model(args, tokenizer, model, dataset: SQLDataset, epoch, device):
...
def evaluate_main(args, tokenizer, model, dataset: SQLDataset, split, epoch, device):
    ...
    with open(os.path.join(args.save, "preds.txt"), "w", encoding="utf-8") as f:
        for q, r in zip(query_strs, response_strs):
            f.write(q.replace("\n", "<n>") + "\t\t" + r.replace("\n", "<n>") + "\n")
            # write_str = q + "\n" + r + "\n"
            # f.write(write_str.encode('utf-8'))
