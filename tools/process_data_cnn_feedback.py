...
class Encoder(object): 
    ...
    def encode(self, line):
        ...
        # response_tokens = Encoder.tokenizer.encode(response, add_special_tokens=False) + [Encoder.tokenizer.eos_token_id]
...
def split_train_pool(args):
    data = []
    with open(os.path.join(args.data_dir, f"train.jsonl")) as fin:
        for line in fin:
    ...
    keep = data[train_sample+1:]
    with open(os.path.join(args.data_dir, f"train.jsonl"), "w+") as fout:
        for _data in train:
            fout.write(json.dumps(_data) + "\n")
    with open(os.path.join(args.data_dir, "keep.jsonl"), "w+") as fout:
        for _data in keep:
            fout.write(json.dumps(_data) + "\n")
...
def process(split: str):
    ...
    if split == 'split':
        split_train_pool(args)
        return None

    with open(os.path.join(args.data_dir, f"{split}.jsonl")) as f:
