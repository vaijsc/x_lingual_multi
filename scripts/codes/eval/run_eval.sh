...
# CUDA_VISIBLE_DEVICES=${DEVICE} bash ${BASE_PATH}/scripts/tiny_llama2/eval/eval_main_lora_cnn.sh ./ ${MASTER_PORT} 1 ${ckpt} $EXP_NAME

# sciq
# CUDA_VISIBLE_DEVICES=${DEVICE} bash ${BASE_PATH}/scripts/tiny_llama2/eval/eval_main_lora_sciq.sh ./ ${MASTER_PORT} 1 ${ckpt} $EXP_NAME

# sql
# CUDA_VISIBLE_DEVICES=${DEVICE} bash ${BASE_PATH}/scripts/codes/eval/eval_main_sql.sh ./ ${MASTER_PORT} 1 ${ckpt} $EXP_NAME
CUDA_VISIBLE_DEVICES=${DEVICE} bash ${BASE_PATH}/scripts/codes/eval/eval_main_lora_sql.sh ./ ${MASTER_PORT} 1 ${ckpt} $EXP_NAME