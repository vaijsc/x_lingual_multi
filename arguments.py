...
def add_model_args(parser: argparse.ArgumentParser):
    ...
    group.add_argument("--teacher-int8", action="store_true")
    group.add_argument("--student-int8", action="store_true")
    return parser
...
def add_peft_args(parser: argparse.ArgumentParser):
    ...
    group.add_argument("--peft-teacher", type=str, default=None)
...
def get_args():
    ...
    if args.type == "eval_main":
        ...
        save_path = os.path.join(
            args.save,
            (f"{ckpt_name}" + f"-{args.peft_name}" if args.peft_name is not None else ""),
