...
default_rouge_scorer = rouge_scorer.RougeScorer(['rougeL', 'rouge1', 'rouge2'], use_stemmer=True)
...
def rougeL_fn(prediction, ground_truth, xlingual=False):
    scorer = default_rouge_scorer
    scores = scorer.score(prediction=prediction, target=ground_truth)
    return scores["rougeL"].fmeasure

def rouge1_fn(prediction, ground_truth, xlingual=False):
    scorer = default_rouge_scorer
    scores = scorer.score(prediction=prediction, target=ground_truth)
    return scores["rouge1"].fmeasure

def rouge2_fn(prediction, ground_truth, xlingual=False):
    scorer = default_rouge_scorer
    scores = scorer.score(prediction=prediction, target=ground_truth)
...
def compute_metrics(predictions, references, xlingual=False):
    ...
    em, rougeL, rouge1, rouge2 = 0, 0, 0, 0
    for pred, gold in zip(predictions, references):
        ...
        rougeL += metric_max_over_ground_truths(
            rougeL_fn, prediction=pred, ground_truths=gold, xlingual=xlingual
        )
        rouge1 += metric_max_over_ground_truths(
            rouge1_fn, prediction=pred, ground_truths=gold, xlingual=xlingual
        )
        rouge2 += metric_max_over_ground_truths(
            rouge2_fn, prediction=pred, ground_truths=gold, xlingual=xlingual
        )
    em = 100.0 * em / len(references)
    rougeL = 100.0 * rougeL / len(references)
    rouge1 = 100.0 * rouge1 / len(references)
    rouge2 = 100.0 * rouge2 / len(references)
    metrics = {"exact_match": em, "rougeL": rougeL, "rouge1": rouge1, "rouge2": rouge2}
